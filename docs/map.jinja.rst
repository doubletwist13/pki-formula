...
We will see a quick setup to configure the ``pki`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``pki`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``pki`` formulas under ``/srv/salt/pki/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/pki/parameters/dns:domain/
    ln -s dns:domain /srv/salt/pki/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/pki/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/pki/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``pki`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``pki`` formulas under ``/srv/salt/pki/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/pki/parameters/roles
...
We create a configuration for the role ``pki/server`` in ``/srv/salt/pki/parameters/roles/pki/server.yaml``:
...
We create another configuration for the role ``pki/client`` in ``/srv/salt/pki/parameters/roles/pki/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles pki/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles pki/client
...
.. code-block:: console
    ...
    mv /srv/salt/pki/parameters/dns:domain/  '/srv/salt/pki/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pki-as-json:
      test.nop:
        - name: {{ pki | json }}
...
Here is an example based on `template-formula/pki/config/file.sls`_:
...
This ``sls`` file expose a ``pki`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pki['winner'] }}
...
.. _template-formula/pki/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/pki/config/file.sls
